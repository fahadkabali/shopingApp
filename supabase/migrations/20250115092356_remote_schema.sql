...
begin
  if new.raw_user_meta_data->>'avatar_url' is null or new.raw_user_meta_data->>'avatar_url' = '' then
    new.raw_user_meta_data = jsonb_set(new.raw_user_meta_data, '{avatar_url}','"https://img.freepik.com/free-vector/young-man-glasses-hoodie_1308-174658.jpg?uid=R135886912&ga=GA1.1.2136420326.1729373480&semt=ais_hybrid"'::jsonb);
  end if;
...
create policy "access to all users"
on "public"."order"
as permissive
for all
to authenticated
...
create policy "Enable delete foradmin users only"
on "public"."product"
as permissive
for delete
to authenticated
...
create policy "Enable update for auth users ONLY"
on "public"."product"
as permissive
for update
to authenticated
